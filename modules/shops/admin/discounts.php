...
if( $nv_Request->isset_request( 'ajax_action', 'post' ) )
{
	...
	if( $new_vid > 0 )
	{
		$sql = 'SELECT did FROM ' . TABLE_SHOPS_NAME . '_discounts WHERE did!=' . $did . ' ORDER BY weight ASC';
		$result = $db->query( $sql );
		$weight = 0;
		while( $row = $result->fetch() )
		{
			...
			$sql = 'UPDATE ' . TABLE_SHOPS_NAME . '_discounts SET weight=' . $weight . ' WHERE did=' . $row['did'];
			$db->query( $sql );
		}
		$sql = 'UPDATE ' . TABLE_SHOPS_NAME . '_discounts SET weight=' . $new_vid . ' WHERE did=' . $did;
		$db->query( $sql );
		$content = 'OK_' . $did;
	}
...
if( $nv_Request->isset_request( 'delete_did', 'get' ) and $nv_Request->isset_request( 'delete_checkss', 'get' ) )
{
	...
	{
		$weight = 0;
		$sql = 'SELECT weight FROM ' . TABLE_SHOPS_NAME . '_discounts WHERE did =' . $db->quote( $did );
		$result = $db->query( $sql );
		list( $weight ) = $result->fetch( 3 );

		$db->query( 'DELETE FROM ' . TABLE_SHOPS_NAME . '_discounts  WHERE did = ' . $db->quote( $did ) );
		if( $weight > 0 )
		{
			$sql = 'SELECT did, weight FROM ' . TABLE_SHOPS_NAME . '_discounts WHERE weight >' . $weight;
			$result = $db->query( $sql );
			while( list( $did, $weight ) = $result->fetch( 3 ) )
			{
				$weight--;
				$db->query( 'UPDATE ' . TABLE_SHOPS_NAME . '_discounts SET weight=' . $weight . ' WHERE did=' . intval( $did ) );
			}
		}
	...
	if( empty( $error ) )
	{
		try
		{
			if( empty( $row['did'] ) )
			{
				...
				$stmt = $db->prepare( 'INSERT INTO ' . TABLE_SHOPS_NAME . '_discounts (title, weight, add_time, edit_time, begin_time, end_time, config) VALUES (:title, :weight, :add_time, :edit_time, :begin_time, :end_time, :config)' );

				$weight = $db->query( 'SELECT max(weight) FROM ' . TABLE_SHOPS_NAME . '_discounts' )->fetchColumn();
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . TABLE_SHOPS_NAME . '_discounts SET title = :title, edit_time = ' . NV_CURRENTTIME . ', begin_time = :begin_time, end_time = :end_time, config = :config WHERE did=' . $row['did'] );
			}
...
elseif( $row['did'] > 0 )
{
	$row = $db->query( 'SELECT * FROM ' . TABLE_SHOPS_NAME . '_discounts WHERE did=' . $row['did'] )->fetch();
	if( empty( $row ) )
	{
		Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op );
		die();
...
if( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	$db->sqlreset()->select( 'COUNT(*)' )->from( '' . TABLE_SHOPS_NAME . '_discounts' );
