...
{
	$stmt = $db->prepare( 'SELECT tid, ' . NV_LANG_DATA . '_image, ' . NV_LANG_DATA . '_description, ' . NV_LANG_DATA . '_keywords FROM ' . TABLE_SHOPS_NAME . '_tags WHERE ' . NV_LANG_DATA . '_alias= :alias' );
	...
	{
		// Fetch Limit
		$db->sqlreset()->select( 'COUNT(*)' )->from( TABLE_SHOPS_NAME . '_rows t1' )->where( 'status=1 AND id IN (SELECT id FROM ' . TABLE_SHOPS_NAME . '_tags_id WHERE tid=' . $tid . ')' );
