...
if( $nv_Request->isset_request( 'checkss', 'get' ) and $nv_Request->get_string( 'checkss', 'get' ) == md5( $global_config['sitekey'] . session_id() ) )
{
	...
	$sql = "SELECT id, catid, exptime FROM " . $db_config['prefix'] . "_" . $module_data . "_rows WHERE id IN (" . implode( ",", $id_array ) . ")";
	$result = $db->query( $sql );

	while( list( $id, $catid, $exptime ) = $result->fetch( 3 ) )
	{
		if( $exptime == 0 or $exptime > NV_CURRENTTIME )
		{
			...
			$arr_catid = array_map( "intval", explode( ",", $catid ) );
		}
	}
