...
if( ! nv_function_exists( 'nv_global_product_center' ) )
{
	...
	function nv_global_product_center( $block_config )
	{
		...
		$db->sqlreset()->select( 't1.id, t1.catid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.homeimgfile, t1.homeimgthumb , t1.homeimgalt' )->from( $db_config['prefix'] . '_' . $mod_data . '_rows t1' )->join( 'INNER JOIN ' . $db_config['prefix'] . '_' . $mod_data . '_block t2 ON t1.id = t2.id' )->where( 't2.bid= ' . $block_config['blockid'] . ' AND t1.status =1' )->order( 't1.id DESC' )->limit( $num_get );

		$list = nv_db_cache( $db->sql(), '', $module );

		foreach( $list as $row )
		{
			$link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module . '&amp;' . NV_OP_VARIABLE . '=' . $array_cat_shops[$row['catid']]['alias'] . '/' . $row['alias'] . $global_config['rewrite_exturl'];
