...
// Tong so san pham
// $number = $db->query( 'SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_' . $mod_data . '_rows where status = 1 AND addtime < ' . NV_CURRENTTIME . ' AND (exptime=0 OR exptime>' . NV_CURRENTTIME . ')' )->fetchColumn();
// if( $number > 0 )
// {
	// $siteinfo[] = array( 'key' => $lang_siteinfo['siteinfo_addtime'], 'value' => $number );
// }

// So san pham cho dang
// $number = $db->query( 'SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_' . $mod_data . '_rows where status = 0 AND addtime < ' . NV_CURRENTTIME . ' AND (exptime=0 OR exptime>' . NV_CURRENTTIME . ')' )->fetchColumn();
// if( $number > 0 )
// {
