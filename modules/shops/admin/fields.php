...
$sql = 'SELECT * FROM ' . TABLE_SHOPS_NAME . '_template where status =1 ';
...
if( $nv_Request->isset_request( 'changeweight', 'post' ) )
{
	...
	$query = 'SELECT COUNT(*) FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $fid;
	$numrows = $db->query( $query )->fetchColumn();
	if( $numrows != 1 ) die( 'NO' );

	$query = 'SELECT fid FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid!=' . $fid . ' ORDER BY weight ASC';
	...
	while( $row = $result->fetch() )
	{
		...
		$sql = 'UPDATE ' . TABLE_SHOPS_NAME . '_field SET weight=' . $weight . ' WHERE fid=' . $row['fid'];
		$db->query( $sql );
	}
	$sql = 'UPDATE ' . TABLE_SHOPS_NAME . '_field SET weight=' . $new_vid . ' WHERE fid=' . $fid;
	$db->query( $sql );
	die( 'OK' );
}
...
if( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( $dataform['fid'] )
	{
		$dataform_old = $db->query( 'SELECT * FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $dataform['fid'] )->fetch();
	...
	else
	{
		...
		else
		{
			...
			$stmt = $db->prepare( 'SELECT * FROM ' . TABLE_SHOPS_NAME . '_field WHERE field= :field' );
	...
	if( empty( $error ) )
	{
		...
		if( empty( $dataform['fid'] ) )
		{
			...
			{
				$weight = $db->query( 'SELECT MAX(weight) FROM ' . TABLE_SHOPS_NAME . '_field' )->fetchColumn();
				$weight = intval( $weight ) + 1;

				$sql = "INSERT INTO " . TABLE_SHOPS_NAME . "_field
				...
				if( $dataform['fid'] )
				{
					...
					{
						$save = $db->exec( "ALTER TABLE " . TABLE_SHOPS_NAME . "_info_" . $templateids_i . " ADD " . $dataform['field'] . " " . $type_date );
						$result = $db->query( "Select field, field_type  FROM " . TABLE_SHOPS_NAME . "_field WHERE  listtemplate IN  (" . $templateids_i . ") " );
					...
					$tablename = TABLE_SHOPS_NAME . "_info_" . $templateids_i;
		...
		elseif( $dataform['max_length'] <= 4294967296 )
		{
			...
			$listtem = $db->query( 'SELECT listtemplate FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $dataform['fid'] )->fetch();

			$arr_t = explode( "|", $listtem['listtemplate'] );

			foreach( $arr_t as $arr_t_i )
			{
				$db->query( "ALTER TABLE " . TABLE_SHOPS_NAME . "_info_" . $arr_t_i . " DROP " . $dataform['field'] );
			}

			$query = "UPDATE " . TABLE_SHOPS_NAME . "_field SET";
			...
			if( $save )
			{
				...
				{
					$result = $db->query( "Select field, field_type  FROM " . TABLE_SHOPS_NAME . "_field WHERE  listtemplate IN  (" . $array_t_i . ") " );
...
if( $nv_Request->isset_request( 'del', 'post' ) )
{
	...
	list( $fid, $listtemplate, $field, $weight ) = $db->query( 'SELECT fid,listtemplate, field, weight FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $fid )->fetch( 3 );
	if( $listtemplate != '' )
	{
		$array_template = explode( "|", $listtemplate );
		foreach( $array_template as $array_template_i )
		{
			$db->query( ' ALTER TABLE ' . TABLE_SHOPS_NAME . '_info_' . $array_template_i . '  DROP ' . $field );
		}
	}
	...
	if( $fid and ! empty( $field ) )
	{
		...
		$query1 = 'DELETE FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $fid;
		if( $db->query( $query1 ) )
		{
			$query = 'SELECT fid FROM ' . TABLE_SHOPS_NAME . '_field WHERE weight > ' . $weight . ' ORDER BY weight ASC';
			$result = $db->query( $query );
			while( $row = $result->fetch() )
			{
				$db->query( 'UPDATE ' . TABLE_SHOPS_NAME . '_field SET weight=' . $weight . ' WHERE fid=' . $row['fid'] );
				++$weight;
			}
			die( 'OK' );
		}
...
if( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	$db->sqlreset()->select( 'COUNT(*)' )->from( '' . TABLE_SHOPS_NAME . '_field' );
...
if( ! isset( $dataform ) )
{
	if( $fid )
	{
		$dataform = $db->query( 'SELECT * FROM ' . TABLE_SHOPS_NAME . '_field WHERE fid=' . $fid )->fetch();
