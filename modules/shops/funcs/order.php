	...
	if( empty( $error ) and $i > 0 )
	{
		...
		if( $order_id > 0 )
		{
			...
			if( ! empty( $arrayid ) )
			{
				...
				$sql = 'SELECT t1.id, t1.catid, t1.addtime, t1.' . NV_LANG_DATA . '_title, t1.' . NV_LANG_DATA . '_alias, t1.' . NV_LANG_DATA . '_hometext, t2.' . NV_LANG_DATA . '_title, t1.money_unit FROM ' . TABLE_SHOPS_NAME . '_rows AS t1 LEFT JOIN ' . TABLE_SHOPS_NAME . '_units AS t2 ON t1.product_unit = t2.id WHERE t1.id IN (' . $templistid . ') AND t1.status =1';
				$result = $db->query( $sql );
				while( list( $id, $catid, $addtime, $title, $alias, $hometext, $unit, $money_unit ) = $result->fetch( 3 ) )
				{
					$data_pro[] = array(
						'id' => $id,
						'addtime' => $addtime,
...
if( $action == 0 )
{
	...
	if( ! empty( $arrayid ) )
	{
		...
		$sql = 'SELECT t1.id, t1.catid, t1.addtime, t1.' . NV_LANG_DATA . '_title, t1.' . NV_LANG_DATA . '_alias, t1.' . NV_LANG_DATA . '_hometext, t1.homeimgfile, t1.homeimgthumb, t1.product_price, t2.' . NV_LANG_DATA . '_title, t1.money_unit, t1.discount_id FROM ' . TABLE_SHOPS_NAME . '_rows AS t1 LEFT JOIN ' . TABLE_SHOPS_NAME . '_units AS t2 ON t1.product_unit = t2.id WHERE t1.id IN (' . $listid . ') AND t1.status =1';
		 $result = $db->query( $sql );
		while( list( $id, $catid, $addtime, $title, $alias, $hometext,  $homeimgfile, $homeimgthumb, $product_price, $unit, $money_unit, $discount_id ) = $result->fetch( 3 ) )
		{
