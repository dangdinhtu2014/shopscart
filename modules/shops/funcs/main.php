			...
			if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
			{
				...
				while( list( $id, $addtime, $title, $alias, $hometext, $homeimgfile, $homeimgthumb, $model, $quantity, $product_price, $money_unit, $discount_id, $showprice, $newday ) = $result->fetch( 3 ) )
				{
					...
					$data_pro[] = array(
						...
						'showprice' => $showprice,
						'newday' => $newday,
						'link_pro' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_cat[$catid_i]['alias'] . '/' . $alias . $global_config['rewrite_exturl'],
						'link_order' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=setcart&amp;id=' . $id );
				}
	...
	elseif( $pro_config['home_view'] == 'view_home_group' )
	{
		...
		foreach( $global_array_group as $groupid_i => $array_info_i )
		{
			if( $array_info_i['parentid'] == 0 and $array_info_i['inhome'] != 0 )
			{
				...
				// Fetch Limit
				$db->sqlreset()->select( 'DISTINCT id' )->from( TABLE_SHOPS_NAME . '_rows t1' )->join( 'INNER JOIN ' . TABLE_SHOPS_NAME . '_items_group t3 ON t3.pro_id = t1.id' )->where( $sql_regexp . ' AND t1.inhome=1 AND t1.status =1' );
